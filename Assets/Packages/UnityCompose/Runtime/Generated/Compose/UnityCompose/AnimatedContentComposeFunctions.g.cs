...
public static partial class ComposeFunctions
{
    ...
    private static void __AnimatedContent<T>(T value, Func<T, T, ContentTransform> transition, [Composable] Action<T> content, bool animateSize = false, float transitionDuration = ComposeDefaults.TransitionDuration, IModifier? modifier = null)
    {
        ...
        try
        {
            ...
            Box<AnimatedContent>(modifier: modifier.OrEmpty().Then(containerModifier), content: RememberComposable<global::System.Action<global::UnityCompose.IBoxScope>>((value, content, isSwitched, resolvedProgress, previousValue, contentModifier, resolvedTransition), scope =>
            {
                var parent = LocalParentLayout.Current;
                var nextModifier = resolvedTransition.Enter.Get(scope, resolvedProgress, parent).Then(contentModifier);
                var previousModifier = resolvedTransition.Exit.Get(scope, resolvedProgress, parent).Float();
                ...
                if (isSwitched.Value || isAnimationRunning)
                {
                    ...
                    {
                        CompositionLocalProvider(provides: IImmutableStableList.Create(LocalModifier.Provides(after: pair.First.Style), LocalTransitionProgress.Provides(pair.First.Progress)), content: RememberComposable<global::System.Action>((content, pair), () => content(pair.First.Value)));
                    }));
                }
                ...
                if (!isSwitched.Value || isAnimationRunning)
                {
                    ...
                    {
                        CompositionLocalProvider(provides: IImmutableStableList.Create(LocalModifier.Provides(after: pair.Second.Style), LocalTransitionProgress.Provides(pair.Second.Progress)), content: RememberComposable<global::System.Action>((content, pair), () => content(pair.Second.Value)));
                    }));
                }
            }));
        }
